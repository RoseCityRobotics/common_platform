...
float PidControl::update(float error) {
  ...
  uint32_t now = micros();
  if (now - lastLog > 20000) { // 50 Hz debug
    SERIAL_OUT.print("PID p="); SERIAL_OUT.print(pTerm);
    SERIAL_OUT.print(" i="); SERIAL_OUT.print(iTerm);
    SERIAL_OUT.print(" d="); SERIAL_OUT.print(dTerm);
    SERIAL_OUT.print(" out="); SERIAL_OUT.println(output);
    lastLog = now;
  }
#endif
